...
/*Main Draw Tree addition*/
  public static void symbols2(PrintWriter out, 
			     boolean emit_non_terms, boolean sym_interface)
    {
      terminal term;
      non_terminal nt;
      ...
      for (Enumeration e = terminal.all(); e.hasMoreElements(); )
	{
      ...
      /*
       * **************************************************
       * ****** BEGIN: CODE SPECIFIC TO MAINDRAWTREE ******
       * **************************************************
       */
      out.println("public String[] TT;");
      out.println("\tpublic sym(){");
      out.println("\t\tTT = new String[100];");
      for (Enumeration e = terminal.all(); e.hasMoreElements(); )
      {
        term = (terminal)e.nextElement();

        /* output a constant decl for the terminal */
        out.println("\t\tTT[" + term.index() + "]=new String(\"" + term.name() + "\");");
      }
      out.println("\t}");
      out.println("public String getTT(int i){return TT[i];}");
      /*
       * **************************************************
       * ******* END: CODE SPECIFIC TO MAINDRAWTREE *******
       * **************************************************
       */
  ...
  protected static void emit_action_code_DT(PrintWriter out, production start_prod)
      ...
      for ( ;proditeration<Math.min((instancecounter+1)*UPPERLIMIT,production.number());prod=(production)production.find(++proditeration) )
	{
	   ...
	   * **************************************************
	   */
	  if (prod.lhs().the_symbol().name() != "$START") {
	    char end = 0;
	    if (prod.lhs().the_symbol().index() == 1)
	      end = '1';
	    else
	      end = '0';

	    /* give them their own block to work in */
	    out.println("            { parser.Tree.reduce(" + prod.rhs_length() + ",\"" + prod.lhs().the_symbol().name() + "\"," + end + ");");
	  }
	  else {
	    /* give them their own block to work in */
	    out.println("            { ");
...
// TUM 20060608: even when its null: who cares?
  ...
  public static void parser2(
      ...
      if (init_code != null)
	{
      ...
      out.println(" Tree = new DisegnaAlbero();   }public static DisegnaAlbero Tree;");
      ...
      if (scan_code != null)
	{
      ...
      out.println(" Symbol s=_scanner.next_token(); Tree.push(s.toString(), 0); return s;   }");
      ...
      if (parser_code != null)
	{
      ...
      /* put out the action code class */
      if (!_xmlactions)
          emit_action_code_DT(out, start_prod);
      else
          emit_xmlaction_code(out, start_prod);
